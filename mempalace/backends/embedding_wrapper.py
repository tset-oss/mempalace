...
class EmbeddingCollection(BaseCollection):
    ...
    def query(
        ...
        *,
        ...
        # Accepted for query-contract parity with the core search path (and the
        # Postgres backend). The ``entity=`` scoping that sets this is
        # postgres-only; the explicit-embedding backends this wrapper fronts
        # (sqlite_exact / qdrant / pgvector) do not honour it, so it is accepted
        # and dropped rather than forwarded to an inner ``query`` that would
        # reject the keyword.
