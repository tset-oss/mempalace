...
class ChromaCollection(BaseCollection):
    ...
    def keyword_candidates(
        ...
        *,
        ...
        # ``{"room": r}`` / ``{"source_file": s}`` for a single filter but
        # ``{"$and": [...]}`` when several are set. A naive ``where.get("wing")``
        # returns None on the ``$and`` shape, dropping the scope — under the
        # union default that leaks out-of-scope keyword candidates into scoped
        # searches (#1815). Flatten the ``$and`` clauses so every filter reaches
        # the FTS5 path.
