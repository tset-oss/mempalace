...
#
# The fork keeps its team-aware WAL inline here (the ``team=`` tag + postgres
# central sink is the team-vault differentiator). Upstream extracted a
# side-effect-free ``mempalace.wal`` for the CLI sync path and daemon service
# layer; that module still exists and serves those consumers (see
# ``cli.py`` / ``service.py``), but mcp_server does NOT import it — importing
# this module installs MCP stdio protection those callers must avoid, and the
# inline version below carries the per-team routing they do not need.
...
def _get_collection(create=False, team=None):
    ...
    override — the single team-resolution point for every server-mode
    read/modify tool. The chroma fast-path below — with its client/collection
    caching and stale-HNSW healing — is preserved unchanged for the local
    default.
    ...
    if _config.backend != "chroma":
        ...
        # Server-mode collections are intentionally NOT cached in the module
        # globals: the active team can change per request (``X-Mempalace-Team``
        # header / ``switch_team``), so each call re-resolves the vault.
...
def tool_check_duplicate(content: str, threshold: float = 0.9):
    ...
    if not col:
        ...
        # chroma: a missing local palace (or a cached open error) is a real
        # error state — surface the richer cached-error-or-no-palace payload.
        return _collection_error_or_no_palace()
    try:
...
def tool_reconnect():
    ...
    close_errors = []
    # Close by bare palace path (RFC-001 ``close_palace`` contract). This is the
    # chroma-only reconnect path — chroma keys its client cache by path and has
    # no team namespace, so the path is sufficient. The team-routed server
    # backends reconnect through ``_tool_reconnect_server`` instead.
    palace_path = _config.palace_path
    closed_backend_names = set()
    cached_backend_name = _collection_cache_backend
    try:
        backend = palace_module.get_backend_for_palace(palace_path)
    ...
    if cached_backend_name and cached_backend_name not in closed_backend_names:
        try:
            ...
            get_backend(cached_backend_name).close_palace(palace_path)
