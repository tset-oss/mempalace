...
def _finalize_format_mine_postloop(*, dry_run, palace_config, wing, palace_path) -> None:
    """Post-loop finalization for a clean ``mine_formats`` run.

    Factored out of ``mine_formats`` to keep the orchestrator under the
    project's mccabe complexity ceiling (max-complexity=25 in pyproject.toml).
    Runs only when every file was processed without interruption. No behavior
    change.

    Computes cross-wing topic tunnels linking this wing to others that share
    confirmed topics (mirrors the post-loop tunnel block in
    ``miner._mine_impl``: tunnel-compute failures must never fail a mine, so any
    exception is logged and skipped quietly). The host-global
    ``~/.mempalace/tunnels.json`` write is gated to chroma — on the central,
    multi-team postgres deployment that single host file is a cross-tenant leak,
    so the server owns and derives the link layer per team instead. Then runs
    the end-of-mine FTS5 integrity check (#1537), which raises
    ``MineValidationError`` to ``cmd_mine`` if ``PRAGMA quick_check`` finds
    malformed FTS5 rows so a corrupted palace cannot silently exit Done on the
    ``--mode extract`` path that bypasses ``_mine_impl``.
    """
...
def mine_formats(
    ...
    try:
        ...
        # All files processed without interruption — run the post-loop
        # finalization (cross-wing topic tunnels + FTS5 integrity check).
        _finalize_format_mine_postloop(
            dry_run=dry_run,
